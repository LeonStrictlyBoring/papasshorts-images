...
export default function ArtikelPage() {
  ...
  useEffect(() => {
    ...
    function onKey(e: KeyboardEvent) { if (e.key === 'Escape') setZoomArtikel(null) }
              ...
              {filtered.map(a => (
                <tr key={a.id} className={`transition-colors ${selected.has(a.id) ? 'bg-orange/5' : 'hover:bg-navy/[0.02]'}`}>
                  ...
                  <td className="px-2 py-2">
                    <button type="button" onClick={() => setZoomArtikel(a)}
          ...
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-7xl h-[90vh]"
            ...
            <div className="p-6 flex gap-6 h-full">
              <div className="flex-1 min-w-0 h-full">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={zoomArtikel.imageUrl} alt={zoomArtikel.produktname} className="w-full h-full rounded-lg object-contain object-left" />
              </div>

              <div className="w-80 shrink-0 flex flex-col justify-end gap-4">
                <div className="space-y-2">
                  {[
                    { label: 'Produktname', value: zoomArtikel.produktname },
                    { label: 'ID',          value: zoomArtikel.artikelId },
                    { label: 'Kategorie',   value: zoomArtikel.kategorie },
                  ].map(({ label, value }) => (
                    <div key={label}>
