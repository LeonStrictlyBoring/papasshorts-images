...
const inputCls = 'border border-navy/20 rounded-md px-3 py-2 text-navy placeholder:text-navy/30 focus:outline-none focus:border-orange'
const selectCls = 'border border-navy/20 rounded-md px-3 py-2 text-navy focus:outline-none focus:border-orange'
...
export default function ModelCreatePage() {
  ...
  function toggleArchetype(label: string) {
    setSelectedArchetypes(prev => { const n = new Set(prev); n.has(label) ? n.delete(label) : n.add(label); return n })
  }
  ...
  function buildRequestPayload(refinement?: { storagePath: string; text: string }) {
    const archetypes = Array.from(selectedArchetypes).map(l => l.split(' (')[0])
    const customArchetype = customArchetypeText.trim() || undefined
    return {
      persona: persona.trim(),
      bodyMeasurements: {
        ...(height && { height }), ...(clothingSize && { clothingSize }), ...(shoeSize && { shoeSize }),
        ...(chest && { chest }), ...(waist && { waist }), ...(hips && { hips }),
      },
      appearance: {
        ...(build && { build }), ...(phenotype && { phenotype }), ...(eyeColor && { eyeColor }),
        ...(skinTone && { skinTone }), ...(skinUndertone && { skinUndertone }),
        ...(hairColor && { hairColor }), ...(hairTexture && { hairTexture }), ...(hairLength && { hairLength }),
        ...(beard && { beard }), ...(specialFeatures.trim() && { specialFeatures: specialFeatures.trim() }),
      },
      archetypes,
    ...
    <div className="max-w-3xl">
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-4">
                <label className="flex flex-col gap-1">
                  ...
                  <input type="number" min={100} max={250} placeholder="z. B. 175" value={height} onChange={(e) => setHeight(e.target.value)} className={inputCls} />
                </label>
                <label className="flex flex-col gap-1">
              ...
              <div className="space-y-4">
                <label className="flex flex-col gap-1">
                  ...
                  <input type="number" min={50} max={200} placeholder="z. B. 90" value={chest} onChange={(e) => setChest(e.target.value)} className={inputCls} />
                </label>
                <label className="flex flex-col gap-1">
                  <span className="text-sm text-navy/70">Taillenumfang (in cm)</span>
                  <input type="number" min={50} max={200} placeholder="z. B. 70" value={waist} onChange={(e) => setWaist(e.target.value)} className={inputCls} />
                </label>
                <label className="flex flex-col gap-1">
                  <span className="text-sm text-navy/70">Hüftumfang (in cm)</span>
                  <input type="number" min={50} max={200} placeholder="z. B. 95" value={hips} onChange={(e) => setHips(e.target.value)} className={inputCls} />
                </label>
              </div>
            </div>
          </section>
          ...
          <section className="space-y-2">
            ...
            {ARCHETYPES.map(label => (
              <label key={label} className="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" checked={selectedArchetypes.has(label)} onChange={() => toggleArchetype(label)} className="w-4 h-4 accent-orange shrink-0" />
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              <input type="text" placeholder="Weitere …" value={customArchetypeText} onChange={(e) => setCustomArchetypeText(e.target.value)} className={`${inputCls} w-full`} />
            </label>
          </section>
        ...
        <div className="space-y-8 bg-navy/5 rounded-xl p-6">
          ...
          {(height || build || clothingSize || shoeSize || chest || waist || hips) && (
            <section className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-3">
                {([['Körpergröße', height ? `${height} cm` : ''],['Statur / Körpertyp', build],['Konfektionsgröße', clothingSize],['Schuhgröße', shoeSize],['Brustumfang', chest ? `${chest} cm` : ''],['Taillenumfang', waist ? `${waist} cm` : ''],['Hüftumfang', hips ? `${hips} cm` : '']] as [string,string][]).filter(([,v]) => v).map(([label, value]) => (
          ...
          {(phenotype || eyeColor || skinTone || skinUndertone || hairColor || hairTexture || hairLength || beard || specialFeatures) && (
            <section className="space-y-3">
              <h3 className="text-base font-bold text-navy uppercase tracking-wide">Optische Merkmale</h3>
              <div className="grid grid-cols-2 gap-x-6 gap-y-3">
                {([['Phänotyp', phenotype],['Augenfarbe', eyeColor],['Hautton', skinTone],['Haut-Unterton', skinUndertone],['Haarfarbe', hairColor],['Haarstruktur', hairTexture],['Haarlänge', hairLength],['Bart', beard]] as [string,string][]).filter(([,v]) => v).map(([label, value]) => (
                  <div key={label}><p className="text-xs text-navy/50">{label}</p><p className="text-sm text-navy">{value}</p></div>
                ))}
                {specialFeatures && <div className="col-span-2"><p className="text-xs text-navy/50">Besondere Merkmale</p><p className="text-sm text-navy">{specialFeatures}</p></div>}
              </div>
            </section>
          )}
          <section className="space-y-2">
            <h3 className="text-base font-bold text-navy uppercase tracking-wide">Model-Typ &amp; Archetyp</h3>
            <p className="text-sm text-navy">{[...Array.from(selectedArchetypes), customArchetypeText.trim()].filter(Boolean).join(', ') || '–'}</p>
          </section>
        </div>
      )}
      ...
      {error && !loading && (
        <div className="mt-8 p-4 border border-red-300 bg-red-50 rounded-lg space-y-3">
          ...
          {!submitted && <button type="button" onClick={handleSubmit} className="text-sm font-medium text-orange hover:text-orange/80 transition-colors">Erneut versuchen</button>}
        </div>
      )}
          ...
          <div className="bg-white rounded-xl p-6 w-80 space-y-4 shadow-xl">
            ...
            <label className="flex flex-col gap-1">
              ...
              <input type="text" value={saveName} onChange={(e) => setSaveName(e.target.value)}
                placeholder="z. B. Business-Typ, Casual …" className={inputCls} autoFocus
